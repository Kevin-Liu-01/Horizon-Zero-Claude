...
export class Retargeter {
  ...
  constructor(o) {
    ...
    // source bones not covered by the map (for the report)
  ...
  /** Pose the target into the source's REST pose (C only) — diagnostic. */
  poseToSourceRest() {
    this.targetRoot.updateMatrixWorld(true);
    this.tInv.copy(this.targetRoot.matrixWorld).invert();
    for (const p of this.pairs) {
      poseIn(p.t.parent, this.tInv, _qP);
      p.t.quaternion.copy(_qP).invert().multiply(p.Wt2);
    }
    this.targetRoot.updateMatrixWorld(true);
  }
   ...
   * @param {object} [opts]  { fps, groundFix=true, extraShift=0 }
   */
  bake(clip, opts = {}) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // ground metric: lowest contact bone vs its bind height
      this.targetRoot.updateMatrixWorld(true);
      let lo = Infinity;
      for (const c of this.contacts) {
        poseIn(c.bone, this.tInv, _q, _v);
        lo = Math.min(lo, _v.y - c.restY);
      }
    ...
    const info = {
      clip: clip.name, duration: +clip.duration.toFixed(3), fps, frames: n,
      hipScale: +this.hipScale.toFixed(4),
      hipRestSource: this.hip.Ps.toArray().map((v) => +v.toFixed(3)),
      hipBindTarget: this.hip.Pt.toArray().map((v) => +v.toFixed(3)),
      ...
      unmappedSource: this.unmappedSource,
