...
- `src/spikes/retarget/boneMap.js` — UAL→Aloy map (52 pairs incl. fingers), Mixamo map (untested), aim rules, contact bones
- `src/spikes/retarget/retarget.js` — `Retargeter` (bake, rest correction, hip scale, root-motion strip, ground metric)
- `src/spikes/retarget/main.js` — viewer page logic (URL params documented in-file), exposes `window.__RETARGET__`
