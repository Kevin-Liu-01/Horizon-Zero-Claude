...
Ownership grant (orchestrator): `player-melee` owns `src/combat/melee.js` (spear pose/attach/swing timing/phase publication), `src/entities/playerAnimator.js`, `src/entities/anim/*` (not anim-core's shared boneSpace/restPose/clipLayer/rigDebug/registry), a new `src/entities/anim/meleeLayer.js` if wanted, `tools/gates.round4.player-melee.mjs`, `docs/ROUND4-PLAYER-MELEE.md`, `docs/research/spear-canon.md`, `reference/spear-*` + `reference/MANIFEST.md` lines. A hook of ≤ 10 lines in `src/combat/combat.js` is allowed only to expose melee phase timing. Grant extended Sep 25 (orchestrator decision): the lane also owns the spear stow transform in `src/combat/combat.js` (`STOW_TILT` and the stow quaternion at the holster socket) — V48 needs it and no in-grant alternative exists. Grant extended again Sep 25 (round 4): the lane may add a melee-specific approach term in `src/core/collision.js` `_syncMachines` (a per-machine reduction of the player standoff radius while `ctx.combat.melee` is drawn/striking, or a lunge-through allowance during the strike phase) so the blade visibly reaches the hull it damages; the general `machinePad` for non-melee movement must not change, and `docs/ROUND4-SPATIAL.md` must record the term. Grant extended Sep 26 (round 5): the lane also owns the ONE line in `src/entities/machines/index.js` that reads `m.meleeStandoffHalfLen ?? m.standoffHalfLen` (and its comment). ORCHESTRATOR RULINGS Sep 26 (from reading V46/V47 myself + the r4 film judge): (R1) light-3 is ONE-HANDED per `docs/research/spear-canon.md` M3 — the left hand never rides the shaft; A101's two-handed clause is void (remove it or make it SKIP with a note), V47 criteria say so. (R2) V46 ready: tip height 0.35–0.55 m char-space (canon M7); the "39° off vertical" claim is orthographic and must be labelled so. (R3) Heavy contact: the fist sits at shoulder height and outboard of the head line (about [-0.22, 1.24, 0.80]); the haft and forearm never cross the face from a dead-front or the V47 row-1 camera; reach comes from spine pitch + lunge, not from raising the hand. (R4) The approach term must release at a bounded rate (no single-frame root jump > 0.15 m) and must be bounded per target by the machine's real hit-hull capsules (her body capsule ≥ 0.05 m from every hull capsule at the resulting standoff); `melee.dispose()` clears it.
